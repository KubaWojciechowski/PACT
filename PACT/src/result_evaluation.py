...
def create_meta_file(seq_list1 : list, seq_list2 : list, output_dir : Path) -> Path:
    ...
    with open(meta_file_path, 'w') as meta_file_handle:
        print("seq,length", file=meta_file_handle)
        for i in range(len(seq_list1)):
            name = "pair_" + str(i)
            length = (len(seq_list1[i])+len(seq_list2[i]))/2
            print("%s,%f" % (name, length), file=meta_file_handle)
